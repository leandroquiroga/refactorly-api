...
from app.domain import (ReviewNotFoundError, ReviewRequest, ReviewResponse)
...
async def review(request: Request, body: ReviewRequest, service: ReviewService = Depends(get_review_service)) -> EventSourceResponse:
    """Submit code for review. Returns SSE stream with LLM chunks"""
...
async def get_history(request: Request, service: ReviewService = Depends(get_review_service)) -> list[ReviewResponse]:
    """Get all past reviews, most recent first"""
    return [ReviewResponse.from_entry(r) for r in await service.get_history()]

@router.get("/{review_id}", response_model=ReviewResponse)
@limiter.limit(settings.HISTORY_RATE_LIMIT)
async def get_review(request: Request, review_id: UUID, service: ReviewService = Depends(get_review_service)) -> ReviewResponse:
    """Get a single review by ID"""
    try:
        return ReviewResponse.from_entry(await service.get_review(str(review_id)))
